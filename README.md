...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Trellis-v2.0-16a34a" alt="Trellis v2.0"/>
...
<p align="center"><img src="docs/img/architecture.svg" alt="Trellis architecture: one drop-in .claude/ folder — constitution, design doc, build journal, findings/lessons ledger, code map, commands, agents, Node hooks, scripts — loaded automatically by Claude Code" width="860"/></p>
...
Trellis ships as a Claude Code **plugin** with namespaced commands — **`/trellis:brainstorm`** (craft the idea), **`/trellis:new`** (scaffold), and **`/trellis:upgrade`** (bring a deployed harness up to the current version). The upgrade path refreshes the *mechanism* files only and **never touches** your journal, ledger, or tailored `CLAUDE.md`. It still installs as a plain skill folder if you prefer no plugin machinery.
...
**The pipeline in one line:** `/trellis:brainstorm` → `/trellis:new` → `/start`.
...
- **Trellis-driven Claude Code owns:** the implementation cycle — decomposition, model routing, the specialist subagents, threat-modeling + review, tests, integration, commits, and the build journal.
...
Trellis ships as both a **versioned plugin** (recommended — updates, namespaced `/trellis:*` commands) and a plain **skill folder**. Both work; pick one.
...
```
Make sure `SKILL.md` ends up directly at `~/.claude/skills/trellis/SKILL.md` (not nested an extra level). If you just created `~/.claude/skills/` for the first time, restart Claude Code so it watches the new directory. This path gives you the auto-triggered skill but not the namespaced `/trellis:*` commands (those are plugin components). Both methods coexist.
</details>

### 2. Craft the idea, then scaffold

```
/trellis:brainstorm     # a principled interview → writes a strong idea.md
/trellis:new            # reads idea.md → writes the tailored .claude/ into your project
```

Already have an `idea.md` (or just want to describe the project)? `/trellis:new` — or *"set up an autonomous build harness for &lt;your idea&gt;"* — runs its own shorter interview and scaffolds directly.
...
A complete, tailored harness — for example, the `.claude/` Trellis produces for a desktop app:
...
Each well-known failure mode of ad-hoc autonomous prompting is matched to a *structural* reason Trellis avoids it — the same mechanisms that carried a real, security-sensitive app end to end:

| Classic failure of "just build it autonomously" | How Trellis structurally prevents it |
|---|---|
...
**Can I update a harness I already scaffolded?** Yes — `/trellis:upgrade` refreshes the *mechanism* files by version stamp and never touches your journal, ledger, or tailored `CLAUDE.md`.
...
├── commands/              plugin-root commands: /trellis:brainstorm · /trellis:new · /trellis:upgrade
